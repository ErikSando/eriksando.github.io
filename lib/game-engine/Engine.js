...
Vector.Inverse = (v) => new Vector(-v.x, -v.y);
...
function drawLine(ctx, start, end, colour = "red", opacity = 1, thickness = 1) {
    ctx.globalAlpha = opacity;
    ctx.strokeStyle = colour;
    ctx.lineWidth = thickness;
    ctx.beginPath();
...
function drawLineObj(ctx, line, colour = "red", opacity = 1, thickness = 1) {
    ctx.globalAlpha = opacity;
    ctx.strokeStyle = colour;
    ctx.lineWidth = thickness;
    ctx.beginPath();
...
class Scene {
    ...
    constructor(GameObjects = [], UIObjects = [], Particles = [], BackgroundImage, BackgroundColour, Gravity, TerminalVelocity) {
        ...
        if (BackgroundImage) {
            ...
            else if (BackgroundImage instanceof String) this.BackgroundImage.src = BackgroundImage;
            else if (BackgroundColour instanceof UIObject) this.BackgroundImage = BackgroundImage.image;
            else console.error("Background image of scene is of an unsupported type. Try using an Image, ImageLabel, or image source.");
        }
...
const Game = new class {
    ...
    PreUpdateGroup = [];
    PostUpdateGroup = [];
    UpdateGroup = []; // keeping this for backwards compatibility
    PreDrawGroup = [];
    PostDrawGroup = [];
    ...
    Update = new _Event(); // same as PostUpdate, but adding this to make it simpler
    PrePreDraw = new _Event();
    PreDraw = new _Event();
    PostDraw = new _Event();
    ...
    Settings = {
        ...
        NativeWidth: 1920,
        NativeHeight: 1080,
        NativeSize: new Vector(1920, 1080),
    ...
    constructor() {
        this.Loaded.AddListener(() => { // What tf did I put this here for ?????? Maybe so Game.Loaded can't be fired by an external script or something
            delete this.Loaded;
        });
    }
    ...
    #Update = (timestamp) => {
        ...
        if (timestamp - this.#lastFPS > this.Settings.FpsUpdateInterval) {
            this.#lastFPS = timestamp;
        ...
        for (let member of this.PreUpdateGroup) if (member.Update) member.Update(delta);
        ...
        for (let member of this.PostUpdateGroup) if (member.Update) member.Update(delta);
    ...
    #Draw = () => {
        ...
        this.PrePreDraw.Invoke(this.ctx);
        ...
        for (let member of this.PreDrawGroup) if (member.Draw) member.Draw(this.ctx);
        ...
        for (let member of this.PostDrawGroup) if (member.Draw) member.Draw(this.ctx);
...
class PreDrawGroup {
    ...
    AddToGame() {
        if (Game.PreDrawGroup.includes(this)) return console.error("Already a part of game, cannot add.");

        Game.PreDrawGroup.push(this);
        this.active = true;
    }
    ...
    RemoveFromGame() {
        ...
        let index = Game.PreDrawGroup.indexOf(this);
        if (index < 0) return console.error("Not apart of game, cannot remove.");
        
        Game.PreDrawGroup.splice(index, 1);
    }
}
...
class PostDrawGroup {
    ...
    AddToGame() {
        if (Game.PostDrawGroup.includes(this)) return console.error("Already a part of game, cannot add.");

        Game.PostDrawGroup.push(this);
        this.active = true;
    }
    ...
    RemoveFromGame() {
        ...
        let index = Game.PostDrawGroup.indexOf(this);
        if (index < 0) return console.error("Not apart of game, cannot remove.");
        
        Game.PostDrawGroup.splice(index, 1);
    }
}

const UpdatesEachFrame = UpdateGroup; // this was the old name, backwards compatibility or something
...
const Input = new class {
    ...
    #CheckMouseOver = (mousePos, uiObject) => {
        ...
        if (uiObject instanceof UIGroup) {
            for (let member of uiObject.members) this.#CheckMouseOver(mousePos, member);
        }
    ...
    #HandleButtonDown = (uiObject, button, e) => {
        if (!this.#CheckButton(this.#MousePos, uiObject)) return false;
    ...
    #HandleButtonUp = (uiObject, button, e) => {
        if (!this.#CheckButton(this.#MousePos, uiObject)) return false;
    ...
    #MouseDown = (e) => {
        ...
        for (let uiObject of Game.scene.UIObjects) {
            if (uiObject instanceof UIGroup) {
                for (let member of uiObject.members) if (this.#HandleButtonDown(member, button, e)) buttonTargeted = true;
                continue;
            }

            if (this.#HandleButtonDown(uiObject, button, e)) buttonTargeted = true;
    ...
    #MouseUp = (e) => {
        ...
        for (let uiObject of Game.scene.UIObjects) {
            if (uiObject instanceof UIGroup) {
                for (let member of uiObject.members) if (this.#HandleButtonUp(member, button, e)) buttonTargeted = true;
                continue;
            }

            if (this.#HandleButtonUp(uiObject, button, e)) buttonTargeted = true;
