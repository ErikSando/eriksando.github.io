...
function CompleteRaycast(start, end, info = new RaycastInfo()) {
    let hits = [];
    let ray = new Line(start, end);
...
function CompleteRaycastOnLayer(start, end, layer, info = new RaycastInfo()) {
    let hits = [];
    let ray = new Line(start, end);
    ...
    for (let gameObject of Game.GetGameObjects()) {
        ...
        if (RectAndLineIntersection(gameObject, ray) && !info.ignoreList.includes(gameObject)) {
            if (layer == gameObject.layer) hits.push(gameObject);
        }
    }
...
function CompleteRaycastByTag(start, end, tag, info = new RaycastInfo()) {
    let hits = [];
    let ray = new Line(start, end);
    ...
    for (let gameObject of Game.GetGameObjects()) {
        ...
        if (RectAndLineIntersection(gameObject, ray) && !info.ignoreList.includes(gameObject)) {
            if (tag == gameObject.tag) hits.push(gameObject);
        }
    }
...
const Input = new class {
    ...
    constructor() {
        ...
        document.addEventListener("keydown", (e) => {
            ...
            else (this.#KeyPressed[e.code]) = false;
        ...
        document.addEventListener("keyup", (e) => {
            ...
            else this.#KeyReleased[e.code] = false;
