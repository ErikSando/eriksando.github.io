...
class AnimationImage {
    constructor(image, offset, scale) {
        this.image = image;
        this.offset = offset;
    ...
    constructor(frames = []) {
        for (let i = 0; i < frames.length; i++) {
            if (frames[i] instanceof Image) {
                frames[i] = new AnimationImage(frames[i], Vector.zero());
            }
        }
...
class GameObject {
    ...
    Update(delta) {
        ...
        if (this.collidable) {
            ...
            this.collisionAbove = false;
            this.collisionBelow = false;
