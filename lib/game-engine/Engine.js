...
const ResizeType = { // Not finished, only aspect ratio works good
    AspectRatio: 1,
    Fullscreen: 2,
    MatchWidth: 3,
    MatchHeight: 4,
...
function fillRect(rect, ctx, colour = "red", opacity = 1) {
    ctx.globalAlpha = opacity;
    ctx.fillStyle = colour;
    ctx.fillRect(rect.position.x, rect.position.y, rect.scale.x, rect.scale.y);
}

function strokeRect(rect, ctx, colour = "red", opacity = 1, thickness = 1) {
    ctx.globalAlpha = opacity;
...
function fillCircle(circle, ctx, colour = "red", opacity = 1) {
    ctx.globalAlpha = opacity;
    ctx.fillStyle = colour;
    ctx.beginPath();
...
function strokeCircle(circle, ctx, colour = "red", opacity = 1, thickness = 1) {
    ctx.globalAlpha = opacity;
    ctx.strokeStyle = colour;
    ctx.lineWidth = thickness;
    ctx.beginPath();
...
function fillTriangle(triangle, ctx, colour = "red", opacity = 1) {
    ctx.globalAlpha = opacity;
    ctx.fillStyle = colour;
    ctx.beginPath();
...
function strokeTriangle(triangle, ctx, colour = "red", opacity = 1, thickness = 1) {
    ctx.globalAlpha = opacity;
    ctx.strokeStyle = colour;
    ctx.lineWidth = thickness;
    ctx.beginPath();
...
function fillPolygon(polygon, ctx, colour = "red", opacity = 1) {
    ctx.globalAlpha = opacity;
    ctx.fillStyle = colour;
    ctx.beginPath();
...
function strokePolygon(polygon, ctx, colour = "red", opacity = 1, thickness = 1) {
    ctx.globalAlpha = opacity;
    ctx.strokeStyle = colour;
    ctx.lineWidth = thickness;
    ctx.beginPath();
...
function fillArcSegment(position, radius = 100, startAngle = 0, endAngle = 180, counterclockwise = false, colour = "red", opacity = 1) {
    ctx.globalAlpha = opacity;
    ctx.fillStyle = colour;
...
function strokeArc(position, radius = 100, startAngle = 0, endAngle = 180, counterclockwise = false, colour = "red", opacity = 1, thickness = 1) {
    ctx.globalAlpha = opacity;
    ctx.strokeStyle = colour;
    ctx.lineWidth = thickness;
    ctx.arc(position.x, position.y, radius, startAngle * DegreesToRadians, endAngle * DegreesToRadians, counterclockwise);
...
class UIObject {
    ...
    Draw(ctx) {
        if (this.outlineThickness > 0 && this.outlineOpacity > 0) {
            ...
            ctx.strokeRect(
                this.position.x - newThickness / 2 + Game.Camera.position.x,
                this.position.y - newThickness / 2 + Game.Camera.position.y,
                this.scale.x + newThickness,
                this.scale.y + newThickness
            );
        }
...
class UIGroup {
    ...
    constructor(members) {
        ...
        this.lastVisible = new Array(this.members.length);

        for (let i = 0; i < this.members.length; ++i) this.lastVisible[i] = this.members[i].visible;
    }
...
class Scene {
    ...
    constructor(GameObjects = [], UIObjects = [], Particles = []) {
...
const Game = new class {
    ...
    Settings = {
        ...
        CanvasResizeType: ResizeType.AspectRatio,
        NativeWidth: 1920,
        NativeHeight: 1080,
    ...
    #ResizeCanvas = () => {
        switch (this.Settings.CanvasResizeType) {
            case ResizeType.AspectRatio: this.#ResizeCanvasAspectRatio(); break;
            case ResizeType.Fullscreen: this.#ResizeCanvasFullscreen(); break;
            case ResizeType.MatchWidth: this.#ResizeCanvasMatchWidth(); break;
            case ResizeType.MatchHeight: this.#ResizeCanvasMatchHeight(); break;
        }
    ...
    #ScaleCtxFullscreen = () => {
        let scale = Math.min(this.canvas.width / this.Settings.NativeWidth, this.canvas.height / this.Settings.NativeHeight);
        this.ctx.setTransform(scale, 0, 0, scale, -this.Camera.position.x * scale, -this.Camera.position.y * scale);
    }
    ...
    #Update = async (timestamp) => {
        ...
        if (!delta || isNaN(delta) || delta < 0 || delta > 0.2) {
            this.#Draw();
            requestAnimationFrame(this.#Update);
            return;
        }
    ...
    #Draw = () => {
        switch (this.Settings.CanvasResizeType) {
            case ResizeType.AspectRatio: this.#ScaleCtxAspectRatio(); break;
            case ResizeType.Fullscreen: this.#ScaleCtxFullscreen(); break;
            case ResizeType.MatchWidth: this.#ScaleCtxMatchWidth(); break;
            case ResizeType.MatchHeight: this.#ScaleCtxMatchHeight(); break;
            default: break;
        }
...
class GameObject {
    ...
    Update(delta) {
        ...
        if (this.usePhysics) {
            this.velocity.x = Lerp(this.velocity.x, 0, this.drag * delta); // not a good way of applying friction, will fix once i find a better way
        }
