...
const Game = new class {
    ...
    #Draw = () => {
        ...
        if (this.Settings.BackgroundImage.src) {
            ...
            this.#ctx.drawImage(image, startX + Game.Camera.position.x, startY + Game.Camera.position.y);

            if (this.Settings.BackgroundImageFit == BackgroundFit.Tile) {
                for (let x = startX + Game.Camera.position.x; x > -image.width; x -= image.width) {
                    this.#ctx.drawImage(image, x, startY + Game.Camera.position.y);
                }

                for (let y = startY + Game.Camera.position.y; y > -image.height; y -= image.height) {
                    this.#ctx.drawImage(image, startX + Game.Camera.position.x, y);
                }

                for (let x = startX + Game.Camera.position.x; x < this.Settings.NativeWidth + Game.Camera.position.x; x += image.width) {
                    this.#ctx.drawImage(image, x, startY + Game.Camera.position.y);
                }

                for (let y = startY + Game.Camera.position.y; y < this.Settings.NativeHeight + Game.Camera.position.y; y += image.height) {
                    this.#ctx.drawImage(image, startX + Game.Camera.position.x, y);
                }
                ...
                for (let x = startX + Game.Camera.position.x + image.width; x < this.Settings.NativeWidth + Game.Camera.position.x; x += image.width) {
                    for (let y = startY + Game.Camera.position.y + image.height; y < this.Settings.NativeHeight + Game.Camera.position.y; y += image.height) {
                        this.#ctx.drawImage(image, x, y);
                    }
                }
...
class GameObject {
    ...
    Update(delta) {
        ...
        for (let gameObject of this.#collisionEnters) {
            this.CollisionEnter.Invoke(gameObject);
        }

        for (let gameObject of this.#collisionExits) {
            this.CollisionExit.Invoke(gameObject);
        }
        ...
        for (let collision in this.#collisionNextFrame) {
            this.collision[collision] = this.#collisionNextFrame[collision];
            this.#collisionNextFrame[collision] = false;
        }
        ...
        if (this.collidable) {
            ...
            for (let gameObject of Game.GetGameObjects()) {
                ...
                if (colliding) this.#newTouchingObjects.push(gameObject);
            }

            for (let gameObject of this.#newTouchingObjects) {
                if (!this.TouchingObjects.includes(gameObject)) this.#collisionEnters.push(gameObject);
            }

            for (let gameObject of this.TouchingObjects) {
                if (!this.#newTouchingObjects.includes(gameObject)) this.#collisionExits.push(gameObject);
            }
        }
...
function Raycast(start, end, ignoreList = [], includeNonCollidable = false) {
    let hit;
    let distance;
    let ray = new Line(start, end);

    for (let gameObject of Game.GetGameObjects()) {
        if (!gameObject.collidable && !includeNonCollidable) continue;
...
function RaycastOnLayer(start, end, ignoreList = [], includeNonCollidable = false, layer) {
    let hit;
    let distance;
    let ray = new Line(start, end);
    ...
    for (let gameObject of Game.GetGameObjects()) {
        ...
        if (RectAndLineIntersection(gameObject, ray) && !ignoreList.includes(gameObject)) {
            if (layer && layer != gameObject.layer) continue;
...
function RaycastByTag(start, end, ignoreList = [], includeNonCollidable = false, tag) {
    let hit;
    let distance;
    let ray = new Line(start, end);
    ...
    for (let gameObject of Game.GetGameObjects()) {
        ...
        if (RectAndLineIntersection(gameObject, ray) && !ignoreList.includes(gameObject)) {
            if (tag && tag != gameObject.tag) continue;
...
const Input = new class {
    ...
    #TouchStart = (e) => {
        ...
        let rect = Game.GetCanvas().getBoundingClientRect();
        let scale = Game.Settings.NativeWidth / canvas.width;
        let mousePos = new Vector(e.changedTouches[0].clientX - rect.x, e.changedTouches[0].clientY - rect.y).multiply(scale);
    ...
    #TouchMove = (e) => {
        ...
        this.#MousePos = new Vector(e.changedTouches[0].clientX, e.changedTouches[0].clientY).multiply(scale);
